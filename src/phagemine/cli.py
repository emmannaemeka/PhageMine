...
Individual installers are also available:
  ...
  phagemine databases install inphared""",
    ...
    for name in ("annotate", "mine", "run", "genbank"):
        ...
        command.add_argument("--reconcile-orfs", action="store_true", help="Deprecated compatibility flag; PHANOTATE/Pyrodigal-gv observation is automatic for fresh DNA analyses")
        command.add_argument("--prodigal", help="Legacy external Prodigal path; never used by the automatic production observer")
    ...
    try:
        ...
        # Fresh genome-analysis commands always request the observational
