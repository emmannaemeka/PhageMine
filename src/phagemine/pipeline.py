...
def run(fasta: str | Path, output: str | Path, command: str = "run", metadata: SubmissionMetadata | None = None, table2asn_executable: str | None = None, predictor: GenePredictor | None = None, representation: GenomeRepresentation | None = None) -> int:
    ...
    manifest = {"pipeline": "PhageMine", "pipeline_version": "0.1.0", "command": command, "input": str(fasta), "input_sha256": checksum(fasta), "genome_representation": representation.manifest(), "gene_caller": {"name": predictor.name, "version": predictor.version(), "parameters": predictor.parameters()}, "evidence_backend": {"name": "mock-phage-evidence", "version": "demo-1", "status": "MOCK; not biological evidence"}}
    quality_control = assess(representation.analysis_sequence, proteins)
    manifest["quality_control"] = quality_control
    write_outputs(output, representation, proteins, candidates, manifest, quality_control, fasta)
    # Local package generation follows annotation, mining, ranking, and QC evidence collection.
    write_package(output, representation.analysis_sequence_id, representation.analysis_sequence, proteins, manifest, metadata, table2asn_executable)
    return len(proteins)
