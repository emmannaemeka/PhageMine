...
def load_pmfdb(path):
    root=Path(path); required=['reference_phage_proteins.faa','reference_metadata.tsv','reference_manifest.json','reference_qc.tsv','mmseqs/target_db.dbtype','mmseqs/mmseqs_index_manifest.json']
    ...
    for name in ['reference_phage_proteins.faa','reference_metadata.tsv','reference_qc.tsv']:
...
def _metadata_records(path, identifiers):
    wanted=set(identifiers); records={}
    if not path or not wanted: return records
    mp=Path(path)
    ...
    with mp.open() as handle:
        for row in csv.DictReader(handle,delimiter='\t'):
            identifier = row.get('external_protein_id') or row.get('protein_id') or row.get('raw_fasta_id') or row.get('id')
...
def validate_external(database, reference=None, output=None, metadata=None, mmseqs='mmseqs', top_n=None, reference_db=None):
    ...
    pmfdb_root=None
    if reference_db:
        pmfdb_root,pmfdb_manifest=load_pmfdb(reference_db); reference=pmfdb_root/'reference_phage_proteins.faa'; metadata=pmfdb_root/'reference_metadata.tsv'
        if Path(output).resolve() == pmfdb_root.resolve() or pmfdb_root.resolve() in Path(output).resolve().parents: raise ValueError('Validation output must not be inside PMFDB release')
    elif not reference:
    ...
    meta=_metadata_records(metadata,{h['external_protein_id'] for h in hits})
    grouped={f['family_id']:[] for f in families}
    for h in hits: grouped.setdefault(h['family_id'],[]).append(h)
    summaries=[]
    for f in families:
        hs=grouped[f['family_id']]; enriched=[]; genomes=set(); hosts=set(); species=set(); taxa=set(); chars=0; predicted=0; incomplete=0
        for h in hs:
            m=meta.get(h['external_protein_id'],{}) or {}; ann=m.get('annotation') or m.get('product') or m.get('existing_annotation'); st=str(m.get('annotation_status','')).lower(); characterized=str(m.get('characterized','')).lower() in {'1','true','yes'} or st in {'experimentally_characterized','experimentally_validated'}; predicted_function=st in {'predicted_function','computational_prediction'} or (bool(ann) and not characterized and st not in {'predicted_uncharacterized','unknown','uncharacterized','hypothetical'}); incomplete += not bool(m) or (not ann and not characterized and st not in {'unknown','uncharacterized','hypothetical','predicted_uncharacterized'})
            h.update({'external_genome_id':m.get('source_genome_id') or m.get('genome_id') or m.get('genome'),'host_genus':m.get('host_genus'),'host_species':m.get('host_species'),'taxon':m.get('phage_taxonomy') or m.get('taxon'),'existing_annotation':ann,'characterized':characterized,'predicted_function':predicted_function,'metadata_incomplete':bool(not m),'provenance':{'search_command':cmd,'mmseqs_executed':True}}); enriched.append(h); genomes.add(h.get('external_genome_id')); hosts.add(h.get('host_genus')); species.add(h.get('host_species')); taxa.add(h.get('taxon')); chars+=bool(h['characterized']); predicted+=bool(h['predicted_function'])
        grouped[f['family_id']]=enriched
        status='NO_EXTERNAL_MATCH' if not hs else ('REFERENCE_METADATA_INCOMPLETE' if incomplete else ('EXTERNAL_CHARACTERIZED_HOMOLOG_FOUND' if chars==len(hs) else ('EXTERNAL_MATCHES_MIXED' if chars else ('EXTERNAL_PREDICTED_FUNCTION_HOMOLOG_FOUND' if predicted else 'EXTERNAL_MATCHES_UNCHARACTERIZED'))))
        best=max(hs,key=lambda x:(x['bitscore'],-x['evalue'])) if hs else {}
        summaries.append({'family_id':f['family_id'],'query_representative':f['representative_protein_id'],'external_match_count':len(hs),'distinct_external_genomes':len({x for x in genomes if x}),'distinct_host_genera':len({x for x in hosts if x}),'distinct_host_species':len({x for x in species if x}),'distinct_taxonomic_groups':len({x for x in taxa if x}),'host_genera':','.join(sorted(x for x in hosts if x)),'best_identity':best.get('identity'),'best_query_coverage':best.get('query_coverage'),'best_target_coverage':best.get('target_coverage'),'best_evalue':best.get('evalue'),'best_bitscore':best.get('bitscore'),'characterized_homolog_count':chars,'predicted_function_homolog_count':predicted,'uncharacterized_homolog_count':len(hs)-chars-predicted,'metadata_incomplete_count':incomplete,'external_validation_status':status})
    ...
    if pmfdb_manifest:
        ...
        prov.update({'phagemine_version':__version__,'pmfdb_version':pmfdb_manifest['pmfdb_version'],'pmfdb_schema_version':pmfdb_manifest['schema_version'],'pmfdb_manifest_sha256':_sha(Path(reference).parent/'reference_manifest.json'),'reference_protein_sha256':_sha(reference),'reference_metadata_sha256':_sha(metadata),'reference_qc_sha256':_sha(Path(reference).parent/'reference_qc.tsv'),'annotation_interpretation':'INPHARED product labels are computational predictions unless independent experimental evidence is explicitly recorded.'})
