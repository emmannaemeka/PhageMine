...
def run_chunked(executable: str, database: str, records: list[tuple[str, str]], *, processes: int, cpus: int, parser, adapter_provenance: dict | None = None):
    ...
    chunks = [records[i::processes] for i in range(processes)]
    with tempfile.TemporaryDirectory(prefix="phagemine-hmmer-chunks-") as td:
        ...
        def worker(index_records):
            ...
            command = [executable, "--cpu", str(cpus), "--domtblout", str(domtbl), database, str(fasta)]
