...
def adjudicate(rows: list[dict], evidence_by_candidate: dict[str, list[dict]] | None = None) -> list[dict]:
    ...
    for row in rows:
        ...
        if conflict=="EXACT_CONCORDANCE": status="CONCORDANT_EVIDENCE"; reason="Both callers produced identical coordinates."; strand_decision="STRAND_SUPPORTS_PHANOTATE"; recommended="PHANOTATE"
        ...
        out.append({**row,"phanotate_orf_support":row.get("phanotate_orf_support", "SUPPORTED" if ps else "UNRESOLVED"),"prodigal_orf_support":row.get("prodigal_orf_support", "SUPPORTED" if ds else "UNRESOLVED"),"start_decision":start_decision,"stop_decision":stop_decision,"strand_decision":strand_decision,"evidence_agreement":"CONCORDANT_EVIDENCE" if ps and ds else ("PARTIALLY_CONCORDANT_EVIDENCE" if ps or ds else "INSUFFICIENT_EVIDENCE"),"phanotate_best_evidence":ps[0] if ps else None,"prodigal_best_evidence":ds[0] if ds else None,"supporting_evidence":{"PHANOTATE":pe,"PRODIGAL":de},"contradictory_evidence":{},"recommended_model":recommended,"adjudication_status":status,"reason":reason,"manual_review":status=="INSUFFICIENT_EVIDENCE"})
    return out
