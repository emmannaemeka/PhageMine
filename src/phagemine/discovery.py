...
def _reports(root, samples, proteins, families, ranking, figures, evidence_reused, pmfdb_provenance, inphared_comparison):
    ...
    if nearest:
        lines += ["", "## INPHARED nearest-reference nucleotide comparison", "", "> Mash selects candidates only. Similarity is PhageMine's bidirectional BLASTN length-normalized calculation, not VIRIDIC output. PhageMine does not assign taxa and withholds boundary interpretation for partial or poorly aligned queries.", "", "| Query | Rank | Reference | Intergenomic similarity (%) | Query aligned (%) | Reference aligned (%) | Host | Reference taxon | Interpretation |", "|---|---:|---|---:|---:|---:|---|---|---|"]
        for row in nearest:
